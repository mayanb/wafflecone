...
class UnusedManager(models.Manager):
    def get_queryset(self):
        return super(UnusedManager, self).get_queryset().filter(inputs__isnull=True,
                                                                creating_task__is_trashed=False).exclude(
            creating_task__process_type__code__in=['SH', 'D'])
